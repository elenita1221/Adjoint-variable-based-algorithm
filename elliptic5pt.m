function  [x,y,dx,dy,u,int_D,params]= elliptic5pt(N,param_vec,param_vec_target,Omega,Mx,My,u_ext,f_src)
...
for j=1:My+2
  if(glbidx(i,j,Mx,My)==0)  %boundary points
    u(i,j)=feval(u_ext,x(i),y(j),N,param_vec);
  end
end
end
...
for j=1:My+2
  ...
  rhs(m1)=feval(f_src,x(i),y(j),param_vec_target);%RHS vector
  rhs(m1)=-rhs(m1);
